function [Tfinal, matcheper, m_best, m1 ] = RANSACTrifocal( vpts1, vpts2, vpts3, matchedTriplets )
%Computing the estimated Trifocal tensor using matches1 and matches2
    ...
    m1 = struct; 
    m_best = struct;
    while matcheper<0.85 && iter<500,
         
        iter = iter+1;
        % each iteration
        idx = randi([1, size(matchedTriplets,1)], 6, 1);
        
        % get x,y coordinates in image 1,2,3 
        % for the six point correspondences
        ...
        if(w1 && w2 && w3 && w4 && w5 && w6)
            ...
            matcheper1 = getErrorTrifocal_ver2(vpts1, vpts2, vpts3, matchedTriplets, Tri);
            iter = iter + 1;
